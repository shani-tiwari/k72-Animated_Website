...
function Navbar() {
    ...
    const menuRefs = [useRef(null), useRef(null)];
    const [navOpen, setNavOpen] = useContext(NavbarContext);
    const [navColor, setNavColor] = useContext(NavbarColorContext);

  return (
    <div className="flex fixed z-4 top-0 justify-between w-full  ">

        <div className=" w-28 p-2  md:w-36 md:p-3">
        ...
        <div onClick={()=>{
            ...
            onMouseEnter={() => { 
                ...
                menuRefs.forEach(ref => ref.current.style.backgroundColor = 'black' )
            }}  
            onMouseLeave={() => {
            ...
            className=" bg-black relative h-11 md:h-13 w-[50vw] md:w-[16.5vw] "
        >

            <div ref={NavGreenRef} className="bg-[#D3FD50] transition-all  absolute top-0 h-0 w-full "></div>

            <div className="relative flex w-full flex-col items-end  justify-center gap-1 mt-[17px] pr-5 ">
                <div ref={menuRefs[0]} className="h-[1.5px] rounded-full  w-10 md:w-16  bg-amber-50"></div>
                <div ref={menuRefs[1]} className="h-[1.5px] rounded-full  w-6 md:w-9 bg-amber-50"></div>
            </div>
